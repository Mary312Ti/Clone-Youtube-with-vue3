<template>
    <div class="relative">
      <router-link :to="{ name: 'PlayingPage', params: { videoId: video.id } }">
        <img
          class="rounded-lg hover:rounded-none w-full h-auto"
      ...
      <span
        class="opacity-100 group-hover:opacity-0 duration-500 absolute bottom-0 right-0 bg-black text-white rounded-md m-1 p-1 text-xs font-semibold"
      >
        6:35
      </span>
    </div>
  ...
  const emitSelectVideo = () => {
    const selectedVideo = video.value;
    if (selectedVideo) {
      emit('selectVideo', selectedVideo);
    }
  };
  </script>
